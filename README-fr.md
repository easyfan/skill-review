...
**Stage 2 (série)** :
- Challenger (sonnet) : émet des verdicts CONFIRM / DISPUTE / UNVERIFIABLE sur les découvertes P0/P1
...
- Stage 2 Challenger : modèle sonnet — environ $0,1–0,5 USD
...
| Élément | Changement |
|---------|------------|
| Step 0g-1 | Nouvelle détection des instances de pattern (`generated-from`) parmi les cibles de revue |
| Reporter Phase 3.5 | Les corrections confirmées sur des instances de pattern sont évaluées pour origine template ; les hits génèrent un proposal de rétro-écriture |
| phase-2b-reporter | Paramètres du Reporter étendus à 11 (generated_from.md + PROJECT_ROOT) |
...
| Élément | Changement |
|---------|------------|
| `agents/phase-2a-challenger.md` | Nouveau : agent d'orchestration Stage 2a |
| `agents/phase-2b-reporter.md` | Nouveau : agent d'orchestration Stage 2b |
| `scripts/check_size.sh` / `read_frontmatter.sh` / `write_self_ref.sh` | Nouveaux : 3 scripts auxiliaires |
| `DESIGN.md` | Nouveau : documentation de conception (classe D) |
| Coordinateur | Référence rapide Stage 1 + tableau Agent + Stage 2 entier remplacés par des appels dispatch |
...
|---------|-------------|
| Isolation par lots | Step 0b supprime les anciens findings à chaque exécution |
| Init pipeline_status.md | Écrasement + sed-replace au lieu d'ajout |
| Contrat de retour Agent | Résumé ≤300 tokens ; findings complets en fichier |
| Référence rapide Stage 1 | Règles critiques en callout avant Stage 1 |
...
| Élément | Changement |
|---------|-----------|
| Mécanisme Gotcha | Challenger accepte `gotcha_context.md` ; les patterns d'échec connus maintiennent des planchers de priorité historiques |
| Modèle Challenger | `model: opus` → `model: sonnet` |
| Discipline d'écriture Bash | Challenger et Reporter utilisent `Bash` heredoc au lieu de l'outil `Write` |
| Mode dégradé Reporter | Nouveau chemin B : ⚪ « non observable » en cas de couverture Challenger partielle |
