...
| Пункт | Изменение |
|-------|-----------|
| Step 0g-1 | Новое обнаружение экземпляров pattern (`generated-from`) среди целей ревью |
| Reporter Phase 3.5 | Подтверждённые исправления в экземплярах pattern проверяются на происхождение из шаблона; совпадения порождают proposal обратной записи |
| phase-2b-reporter | Параметры Reporter расширены до 11 (generated_from.md + PROJECT_ROOT) |
...
| Пункт | Изменение |
|-------|-----------|
| `discover_targets.sh` | Новое: разрешение целей через индекс поля `name` из frontmatter с поддержкой пространств имён (`po:release`) |
| `load_gotchas.sh` | Определение имени skill теперь предпочитает поле `name` из frontmatter |
| `init_scratch.sh` | Блокировка параллельного запуска переведена на чисто временную семантику |
...
| Элемент | Изменение |
|---------|-----------|
| Изоляция пакетов | Step 0b удаляет старые findings перед каждым запуском |
| Инициализация pipeline_status.md | Перезапись + sed-replace вместо дозаписи |
| Контракт возврата Agent | Итог ≤300 токенов; полные findings в файле |
| Быстрая справка Stage 1 | Критические правила вынесены в callout перед Stage 1 |
